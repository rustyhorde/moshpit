#!/bin/sh
# Runs inside the repomon linux-build-aarch64 job (see .repomon/release.toml)
...
# variants for aarch64-unknown-linux-musl, matching the former GitHub Actions
# release.yml `build` job's aarch64 leg — minus the ssh-agent-piggyback-only
...
#
# The moshpit-cross-aarch64 Docker image bundles static libfido2/libcbor/
# libusb/hidapi/libudev-zero for the FIDO2 unlock backend (see
# docker/Dockerfile.aarch64-unknown-linux-musl and Cross.toml) — plain
...
docker build \
    -t moshpit-cross-aarch64 \
    -f docker/Dockerfile.aarch64-unknown-linux-musl \
    docker/
